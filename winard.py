...
#            Bag.Coord.clear()
        ...
        if   t==24:  
            ...
            Bag.token = Bag.data[1]
            if Bag.token == 26: 
                Bag.rbuttonNum  = Bag.data[2]  
                selectRadioButton()
            if Bag.token == 50: 
                Bag.mrbuttonNum  = Bag.data[2]  
                selectMenuRadioButton()
            if Bag.token == 54: 
                Bag.bmradioNum  = Bag.data[2]  
                selectBarMenuRadioButton()
...
# messages            
        ...
        Bag.transmit="128"
#        print("Bag.transmit = ",Bag.transmit)
...
def transceiver_start():
    Bag.transmit="13" 
    while Bag.transmit!="128": 
        Bag.data_b = arduino.readline()
        l=len(Bag.data_b)
        print("startup condition: ",Bag.transmit)
        if l>0:
            parser()
            if Bag.transmit=="128 ":
                arduino.write(bytes(Bag.transmit,'utf-8'))   
            print("startup received: ",Bag.transmit)
    transceiver2()    
...
def transceiver2():  
    ...
    if Bag.transmit != "\0":
        arduino.write(bytes(Bag.transmit,'utf-8'))
        Bag.transmit="\0"
    exec(f"Objects.{root}.after(Bag.td, transceiver2)")
...
#    exec(f"Objects.{root}=ThemedTk(theme='default')")
    exec(f"Objects.{root}.after(200, transceiver_start)")
...
# as usual with a function().
    exec(f"def {entry}(b):\n  txt=Objects.{entry}.get()\n  tkn = {token}\n  enum = {num}\n  transmitE(tkn,enum,txt)")     # print(tkn,enum,txt)")      
...
def createSpinbox():
    ...
    if Bag.Stype == '0':
        ...
        Bag.Sformat    = spin[3]   
        exec(f"Objects.{spinbox} = Spinbox(Objects.{dest}, from_=Bag.Sfrom, to=Bag.Sto, increment=Bag.Sincrement, format=Bag.Sformat, width=Bag.width1, justify=Bag.Sjustify, {font} textvariable= var, command=partial(transmitSB, t=Bag.token, n=Bag.tokenNum, v=var))")   
    if Bag.Stype == '1':  
        exec(f"Objects.{spinbox} = Spinbox(Objects.{dest}, values=Bag.text, format=Bag.Sformat, width=Bag.width1, justify=Bag.Sjustify, {font} textvariable= var, command=partial(transmitSB, t=Bag.token, n=Bag.tokenNum, v=var))")       
...
def createRadioButton():
    ...
    v=Bag.tokenNum 
    vstr=str(v)
    exec(f"Objects.{rbutton} = Radiobutton(Objects.{dest}, text=Bag.text, variable={Bag.rbuttonVar}, value=vstr,  command=partial(transmitB, t=Bag.token, n=Bag.tokenNum, v=v))")   
    exec(f"Objects.{rbutton}['style'] = trbutton")        
    exec(f"Objects.{rbutton}.place(x={Bag.X1},y={Bag.Y1})")
     
#########################################
def selectRadioButton():
    Bag.rbuttonVar="StrVars.rbutton"+str(Bag.rbuttonNum)
    exec(f"{Bag.rbuttonVar}=StringVar(value=str(Bag.rbuttonNum))")
    if Bag.prt == 1: print("    select Rbutton"+str(Bag.rbuttonNum) )
 
#########################################
def selectMenuRadioButton():
    Bag.mrbuttonVar="StrVars.mrbutton"+str(Bag.mrbuttonNum)
    exec(f"{Bag.mrbuttonVar}=StringVar(value=str(Bag.mrbuttonNum))")
    if Bag.prt == 1: print("    select mrbutton"+str(Bag.mrbuttonNum) )
 
#########################################
def selectBarMenuRadioButton():
    Bag.bmradioVar="StrVars.bmradio"+str(Bag.bmradioNum)
    exec(f"{Bag.bmradioVar}=StringVar(value=str(Bag.bmradioNum))")
    if Bag.prt == 1: print("    select bmradio"+str(Bag.bmradioNum) )
...
#    v=Bag.mrbuttonNum 
    v=Bag.tokenNum 
    vstr=str(v)
    exec(f"Objects.{menu}.add_radiobutton(label=Bag.text, value=vstr, variable={Bag.mrbuttonVar}, command=partial(transmitB, t=Bag.token, n=Bag.mrbuttonNum, v=v))")
...
def barmenu_add_radiobutton():    # tk
    ...
    exec(f"Objects.{dest}.add_radiobutton(label=Bag.text {Bag.menuconfig3}, indicatoron=Bag.indon, variable={Bag.bmradioVar}, value=vstr, command=partial(transmitB, Bag.token, Bag.tokenNum, 0))")
...
def transmitB(t,n,v): # buttons 
    ...
    if (Bag.prt == 2): print (Bag.transmit)
#    print ("##",Bag.transmit)
...
def transmitSB(t,n,v): # spinbox 
    Bag.transmit="2 "+str(t)+" "+str(n)+" "+v.get()
    if (Bag.prt == 2): print (Bag.transmit)   
    
def transmitE(t,n,s): # entry
    Bag.transmit="2 "+str(t)+" "+str(n)+" "+s
    if (Bag.prt == 2): print (Bag.transmit)    
